    ...
    if (__HAL_DSI_GET_IT_SOURCE(&hdsi_eval, DSI_IT_ER))
    {
        ...
        if (bitDepth == 24)
        {
            if (updateRegion == 0)
            {
                ...
                LTDC_Layer1->CFBAR = ((uint32_t)currFbBase) + (((uint32_t)HAL::FRAME_BUFFER_WIDTH * HAL::FRAME_BUFFER_HEIGHT / 2) * 3);
        ...
        else
        {
            ...
            {
                DSI->WCR &= ~(DSI_WCR_DSIEN);
                LTDC_Layer1->CFBAR = ((uint32_t)currFbBase) + HAL::FRAME_BUFFER_WIDTH * HAL::FRAME_BUFFER_HEIGHT / 2 * updateRegion;
                LTDC->SRCR = (uint32_t)LTDC_SRCR_IMR;
                LCD_SetUpdateRegion(updateRegion);
                DSI->WCR |= DSI_WCR_DSIEN;
                HAL_DSI_Refresh(&hdsi_eval);
