...
__attribute__((section (".sdram"))) QTNode _nodefactory[STAR_CATALOG_SIZE + STAR_NAMED_STAR_COUNT];
...
QTNode* QTNode::malloc() {
	if (_nodecount == STAR_CATALOG_SIZE + STAR_NAMED_STAR_COUNT)
		return NULL;
...
void StarCatalog::query_common(void (*cb)(StarInfo*, void*), float ramin,
		float ramax, float decmin, float decmax, void *arg, float maxmag) {
	qt_common.query(cb, ramin, ramax, decmin, decmax, arg, maxmag);
}

void StarCatalog::query_all(void (*cb)(StarInfo*, void*), float ramin,
		float ramax, float decmin, float decmax, void *arg, float maxmag) {
...
void QuadTree::query(void (*cb)(StarInfo*, void*), float ral, float rar,
...
StarInfo* QuadTree::_find(QTNode *p, float ra, float dec, float &maxdist) {
	...
	float d2 = sqr(remainderf(p->star->DEC - dec, 360))
			+ sqr(remainderf(p->star->RA - ra, 360));
...
void QuadTree::_query(void (*cb)(StarInfo*, void*), QTNode *p, float ral,
