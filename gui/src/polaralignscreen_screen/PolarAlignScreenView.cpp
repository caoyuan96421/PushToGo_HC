		...
		buttonCallback(this, &PolarAlignScreenView::buttonPressed), starSelectedCallback(this, &PolarAlignScreenView::starSelected)
{
	...
	time_now = TelescopeBackend::getTime();
	minAlt = 5;
	height = 40;
	num_stars = 0;
	selectedStar = NULL;
...
void PolarAlignScreenView::_callback(SkyObjInfo* star)
{
	...
	AzimuthalCoordinates az = eq.toLocalEquatorialCoordinates(time_now, loc).toAzimuthalCoordinates(loc);
...
void PolarAlignScreenView::starSelected(const AbstractButton& src)
{
	if (const_cast<AbstractButton &>(src).getParent() == &container_stars)
	{
		...
		LocalEquatorialCoordinates leq = eq.toLocalEquatorialCoordinates(time_now, loc);
...
{
	if (&src == &this->buttonRestart && pa_state != PA_ERROR)
	{
		listStars();
	}
