...
static const float SELECTION_ERROR_THRESHOLD = 40.0f;
...
static float getMaxMagnitude(float fovw) {
...
StarMapWidget::StarMapWidget() :
	...
	for (PlanetMoon::Object obj = PlanetMoon::MERCURY; obj <= PlanetMoon::MOON;
			obj = (PlanetMoon::Object) (obj + 1)) {
		strncpy(planetSunMoon[(int) obj].name, PlanetMoon::getName(obj),
				sizeof(planetSunMoon[(int) obj].name));
	...
	moveAnimationEquation = EasingEquations::cubicEaseInOut;
...
void StarMapWidget::handleClickEvent(const ClickEvent &evt) {
	if (evt.getType() == ClickEvent::PRESSED) {
		...
		ispressed = true;
		if (!first && !isdrag
				&& (get_millisec_time() - tim) < DOUBLE_CLICK_THRESHOLD_MS
				&& lastClickDuration < DOUBLE_CLICK_DURATION_MS) {
			isdoubleclick = true;
		}
		isdrag = false;
		clickStartTime = get_millisec_time();
		clickX = evt.getX();
		clickY = evt.getY();
	} else {
		// Release
		if (!isdrag) {
			...
			float xs = p * xc
					+ q
							* (-cosf(theta) * sinf(dec_ctr * DEGREE_F)
									* cosf(ra_ctr * DEGREE_F)
									+ sinf(theta) * sinf(ra_ctr * DEGREE_F));
			float ys = p * yc
					+ q
							* (-cosf(theta) * sinf(dec_ctr * DEGREE_F)
									* sinf(ra_ctr * DEGREE_F)
									- sinf(theta) * cosf(ra_ctr * DEGREE_F));
			...
			SkyObjInfo *newselected = NULL;
			// First check planets and Sun/Moon
			for (PlanetMoon::Object obj = PlanetMoon::MERCURY;
					obj <= PlanetMoon::MOON; obj =
							(PlanetMoon::Object) (obj + 1)) {
			...
			// Then check stars
			if (!newselected) {
				newselected = StarCatalog::getInstance().searchByCoordinates(ra,
						dec, maxdist, false, getMaxMagnitude(fovw));

				// Find it in visible star list
				bool found = false;
				for (int i = 0; i < this->num_stars; i++)
					if (visibleStars[i].info == newselected) {
						found = true;
						break;
					}
...
//				debug_if(SMW_DEBUG, "Selected star: id=%d, ra=%f, dec=%f\r\n", selected->id, selected->RA, selected->DEC);
		...
		ispressed = false;
		isdoubleclick = false;
		lastClickDuration = get_millisec_time() - clickStartTime;
		tim = get_millisec_time();
	}
	first = false;
}
...
//	uint64_t time_start = tim.read_high_resolution_us();
	...
	if (displayMoon) {
		// Set moon position
		moon_texture.setXY((moonPos.x - moonPos.size).to<int>(),
				(moonPos.y - moonPos.size).to<int>());
		moon_texture.setWidth((moonPos.size * 2).to<int>());
		moon_texture.setHeight((moonPos.size * 2).to<int>());
		moon_texture.setScale(
				(moonPos.size * 2).to<float>() / (moon_bitmap.getWidth() + 4));
		...
		if ((fontToDraw != 0) && typedText.hasValidId()) {
			for (int i = 0; i < num_label; i++) {
				...
				if (!dirty.isEmpty()) {
					...
					LCD::StringVisuals visuals(fontToDraw, labelColor,
							labelAlpha, typedText.getAlignment(), 0,
							TEXT_ROTATE_0, typedText.getTextDirection(), 0,
							WIDE_TEXT_NONE);
...
void StarMapWidget::handleTickEvent() {
	static int count = 0;
	time_t newtime = TelescopeBackend::getTime();
...
//		debug_if(SMW_DEBUG, "tick\r\n");
	...
	if (moveAnimationRunning) {
		if (moveAnimationCounter < (uint32_t) (moveAnimationDuration)) {
			// Adjust the used animationCounter for the startup delay

			double deltaX = moveAnimationEquation(moveAnimationCounter, 0,
					(int16_t) ((moveAnimationEndX - moveAnimationStartX) * 64),
					moveAnimationDuration) / 64.0;
			double deltaY = moveAnimationEquation(moveAnimationCounter, 0,
					(int16_t) ((moveAnimationEndY - moveAnimationStartY) * 64),
					moveAnimationDuration) / 64.0;

			ra_ctr = moveAnimationStartX + deltaX;
			dec_ctr = moveAnimationStartY + deltaY;
			moveAnimationCounter++;
		} else {
			// Final set
			ra_ctr = moveAnimationEndX;
			dec_ctr = moveAnimationEndY;
			moveAnimationRunning = false;
			if (moveAnimationCallback && moveAnimationCallback->isValid()) {
				moveAnimationCallback->execute();
			}
		}
		updateView();
		invalidate();
...
void StarMapWidget::updateView() {
	...
	if (dec_ctr + fovr >= 85.0f) {
		...
		StarCatalog::getInstance().query_stars(StarMapWidget::callback, -180.0f,
				180.0f, dec_ctr - fovr, 90.0f, (void*) this, maxmagnitude);
		StarCatalog::getInstance().query_dso(StarMapWidget::callback, -180.0f,
				180.0f, dec_ctr - fovr, 90.0f, (void*) this, maxmagnitude);
		...
		float delta_ra = atanf(
				sqrtf(1.0f + tanf(maxdec * DEGREE_F) * tanf(maxdec * DEGREE_F))
						* tanf(fovr * DEGREE_F)) * RADIAN_F;

		StarCatalog::getInstance().query_stars(StarMapWidget::callback,
				remainderf(ra_ctr - delta_ra, 360.0f),
				remainderf(ra_ctr + delta_ra, 360.0f), dec_ctr - fovr,
				dec_ctr + fovr, (void*) this, maxmagnitude);

		StarCatalog::getInstance().query_dso(StarMapWidget::callback,
				remainderf(ra_ctr - delta_ra, 360.0f),
				remainderf(ra_ctr + delta_ra, 360.0f), dec_ctr - fovr,
				dec_ctr + fovr, (void*) this, maxmagnitude);
	}
	...
	for (PlanetMoon::Object obj = PlanetMoon::MERCURY; obj <= PlanetMoon::MOON;
			obj = (PlanetMoon::Object) (obj + 1)) {
		EquatorialCoordinatesWithDist eq = PlanetMoon::calculatePosition(obj,
				timestamp, location);
		...
		if (planetSunMoon[(int) obj].magnitude <= maxmagnitude) {
			if ((obj != PlanetMoon::SUN && obj != PlanetMoon::MOON)
					|| fovw >= 20.0f) {
				// Handle as star
				_handleStarsAndDSO(&planetSunMoon[(int) obj], true);
			} else if (obj == PlanetMoon::SUN) {
				// Handle Sun (zoomed in)
				_handleSun(&planetSunMoon[(int) obj]);
			} else {
				// Handle Moon (zoomed in)
				planetSunMoon[(int) obj].phase = PlanetMoon::getLunarPhase(
						timestamp, location, eq);
	...
	if (selected) {
		...
		if (!found) {
			selected = NULL;
			if (selectionCallback) {
				selectionCallback->execute(NULL);
			}
		}
	}
}

void StarMapWidget::callback(SkyObjInfo *star, void *arg) {
	StarMapWidget *pw = (StarMapWidget*) arg;
	cb_count++;
	if (!pw)
		return;

	pw->_handleStarsAndDSO(star, false);
}

bool StarMapWidget::_calcScreenPosition(const SkyObjInfo *star, bool hires,
		CWRUtil::Q5 &xscr, CWRUtil::Q5 &yscr, float maxradius) const {
	if (!hires) {
...
//		float rf = acosf(clip(dotsc)) * RADIAN_F; // Angular distance to field centerfloat x1 = c1 * c3; <- This is inaccurate for small angle deviations, especially for single-precision
		float xt = ys * zc - yc * zs;
		float yt = zs * xc - zc * xs;
		float zt = xs * yc - xc * ys;

		float dotsc = xc * xs + yc * ys + zc * zs; // Dot product
		float crosssc = sqrtf(xt * xt + yt * yt + zt * zt); // Cross product
		float rf = atan2f(crosssc, dotsc) * RADIAN_F;
		...
		// FIXed: when star is exactly aligned with aim direction, ljm1 will be divide by zero
		float ljm1 = 1.0f / (sqrtf(xj * xj + yj * yj + zj * zj) + FLT_EPSILON); // 1/Vector length, for normalization
		...
		double xs = cos(ss.accurate_pos.dec * DEGREE)
				* cos(ss.accurate_pos.ra * DEGREE);
		double ys = cos(ss.accurate_pos.dec * DEGREE)
				* sin(ss.accurate_pos.ra * DEGREE);
		...
		double Y = -cos(ss.accurate_pos.dec * DEGREE)
				* sin((ss.accurate_pos.ra - ra_ctr) * DEGREE);
		double X = -sin(dec_ctr * DEGREE) * cos(ss.accurate_pos.dec * DEGREE)
				* cos((ss.accurate_pos.ra - ra_ctr) * DEGREE)
				+ cos(dec_ctr * DEGREE) * sin(ss.accurate_pos.dec * DEGREE);
...
void StarMapWidget::_handleStarsAndDSO(const SkyObjInfo *star,
		bool hires) const {
...
//	debug_if(SMW_DEBUG, "xf=%f, yf=%f\r\n", xf, yf);
	...
	colortype starColor = Color::getColorFrom24BitRGB(
			uint8_t(red / norm * 255.0f), uint8_t(green / norm * 255.0f),
			uint8_t(blue / norm * 255.0f));
	...
	if (num_label < STARMAP_WIDGET_MAX_LABEL) {
		switch (star->type) {
		case SkyObjInfo::SKYOBJ_STAR:
			if (*star->name != '\0') {
				// Generate name label
				strncpy(starlabels[num_label].label, star->name,
						STARMAP_WIDGET_MAX_LABEL_LENGTH);
				starlabels[num_label].x = label_x;
				starlabels[num_label].y = label_y;
				num_label++;
			}
			break;
		case SkyObjInfo::SKYOBJ_MESSIER:
			// Generate name label
			if (*star->name != '\0')
				snprintf(starlabels[num_label].label,
						STARMAP_WIDGET_MAX_LABEL_LENGTH, "M%d %s", star->id,
						star->name);
			else
				snprintf(starlabels[num_label].label,
						STARMAP_WIDGET_MAX_LABEL_LENGTH, "M%d", star->id);
			starlabels[num_label].x = label_x;
			starlabels[num_label].y = label_y;
			num_label++;
			break;
...
void StarMapWidget::_handleSun(const SolarSystemInfo *sun) const {
	...
	if (num_label < STARMAP_WIDGET_MAX_LABEL) {
		// Generate name label
		strncpy(starlabels[num_label].label, sun->name,
				STARMAP_WIDGET_MAX_LABEL_LENGTH);
		starlabels[num_label].x = label_x;
		starlabels[num_label].y = label_y;
		num_label++;
...
void StarMapWidget::_handleMoon(const SolarSystemInfo *moon) const {
	...
	if (num_label < STARMAP_WIDGET_MAX_LABEL) {
		// Generate name label
		strncpy(starlabels[num_label].label, moon->name,
				STARMAP_WIDGET_MAX_LABEL_LENGTH);
		starlabels[num_label].x = label_x;
		starlabels[num_label].y = label_y;
		num_label++;
...
//	this->canvas = &canvas;
	...
	for (int i = 0; i < num_stars; i++) {
		...
		Rect bb((s.x - s.size).to<int>(), (s.y - s.size).to<int>(),
				(s.size * 2).to<int>(), (s.size * 2).to<int>());
...
void StarMapWidget::_drawstar(touchgfx::Canvas &canvas, StarPos &star) const {
	...
	for (int i = 0; i <= 360; i += step) {
		canvas.lineTo(star.x + star.size * CWRUtil::sine(i),
				star.y + star.size * CWRUtil::cosine(i));
	}
...
void StarMapWidget::_drawmoon(touchgfx::Canvas &canvas) const {
	...
	canvas.moveTo(moonPos.x + moonPos.size * CWRUtil::sine(moonPos.apex),
			moonPos.y - moonPos.size * CWRUtil::cosine(moonPos.apex));
	for (int i = 180; i <= 360; i += di) {
		canvas.lineTo(
				moonPos.x + moonPos.size * CWRUtil::sine(i + moonPos.apex),
				moonPos.y - moonPos.size * CWRUtil::cosine(i + moonPos.apex));
	}

	// Draw the dark fraction
	for (int i = 0; i <= 180; i += di) {
		CWRUtil::Q5 dx = (moonPos.size * CWRUtil::sine(i))
				* CWRUtil::cosine(moonPos.illumangle);
		CWRUtil::Q5 dy = moonPos.size * CWRUtil::cosine(i);

		canvas.lineTo(moonPos.x + dx * cosa + dy * sina,
				moonPos.y + dx * sina - dy * cosa);
	}
...
void StarMapWidget::_drawsun(touchgfx::Canvas &canvas) const {
	...
	for (int i = 0; i <= 360; i += 10) {
		canvas.lineTo(sunPos.x + sunPos.size * CWRUtil::sine(i),
				sunPos.y - sunPos.size * CWRUtil::cosine(i));
	}
	...
	for (int i = 0; i <= 360; i += 20) {
		canvas.lineTo(sunPos.x + sunPos.size * CWRUtil::sine(i),
				sunPos.y - sunPos.size * CWRUtil::cosine(i));
		canvas.lineTo(sunPos.x + size2 * CWRUtil::sine(i + 10),
				sunPos.y - size2 * CWRUtil::cosine(i + 10));
	}
...
void StarMapWidget::_drawticks(touchgfx::Canvas &canvas, CWRUtil::Q5 x0,
		CWRUtil::Q5 y0, CWRUtil::Q5 r) const {
...
//	starpainter.setColor(Color::getColorFrom24BitRGB(0, 75, 165), 180);
	...
	while ((sg = SkyCulture::nextSegment(it)) != NULL) {
		...
		float cp1 = xg * (y1 * zj - z1 * yj) + yg * (z1 * xj - x1 * zj)
				+ zg * (x1 * yj - y1 * xj); // Sin of angle between proj and star1
		float cp2 = xg * (y2 * zj - z2 * yj) + yg * (z2 * xj - x2 * zj)
				+ zg * (x2 * yj - y2 * xj); // Sin of angle between proj and star2
		...
		for (int i = 0; i <= nstep; i++) {
			...
			if (i != 0 && (r.intersect(xscr, yscr) || r.intersect(x0, y0))) {
				...
				_drawline((int) x0, (int) y0, (int) xscr, (int) yscr,
						invalidatedArea,
						Color::getColorFrom24BitRGB(160, 85, 0), fbuf);
			}
...
void StarMapWidget::_drawline(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
		const Rect &invalid, colortype color, uint8_t *fb) const {
