...
StarMapWidget::StarMapWidget() :
		...
		true), clickStartTime(0), lastClickDuration(0), clickX(-1000), clickY(-1000), timestamp(TelescopeBackend::getTime()), num_label(0), num_stars(0)
{
...
void StarMapWidget::handleClickEvent(const ClickEvent& evt)
{
	if (evt.getType() == ClickEvent::PRESSED)
	{
		// Press
		ispressed = true;
		if (!first && !isdrag && (get_millisec_time() - tim) < DOUBLE_CLICK_THRESHOLD_MS && lastClickDuration < DOUBLE_CLICK_DURATION_MS)
		{
			isdoubleclick = true;
		}
		isdrag = false;
...
bool StarMapWidget::_calcScreenPosition(const StarInfo *star, bool hires, CWRUtil::Q5 &xscr, CWRUtil::Q5 &yscr, float maxradius) const
{
	if (!hires)
	{
		...
		float ljm1 = 1.0f
				/ (sqrtf(xj * xj + yj * yj + zj * zj) + FLT_EPSILON ); // 1/Vector length, for normalization
