...
void MountScreenView::trackToggled(const AbstractButton& src)
{
	...
	{
		// Start tracking
		TelescopeBackend::track(true);
	}
	else
	{
		TelescopeBackend::track(false);
	}
}
...
void MountScreenView::trackSpeedSelected(const AbstractButton& src)
{
	...
	{
		TelescopeBackend::setSpeed("track", 1.0);
	}
	else if (&src == &toggleLunar)
	{
		TelescopeBackend::setSpeed("track", lunar_speed / sidereal_speed);
	}
	else if (&src == &toggleSolar)
	{
		TelescopeBackend::setSpeed("track", solar_speed / sidereal_speed);
	}
...
void MountScreenView::slewSpeedClicked(const TextAreaWithOneWildcard&, const ClickEvent& evt)
{
	if (evt.getType() == ClickEvent::RELEASED && cpop.isVisible() == false)
	{
		...
		config_edit.value.ddata = TelescopeBackend::getSpeed("slew");
		config_edit.min.ddata = 0;
		config_edit.max.ddata = TelescopeBackend::getConfigDouble("max_speed");
...
void MountScreenView::eqCoordGoto(CoordinatePopup::Coordinate c, bool ok)
{
	...
	{
		char buf[64];
		EquatorialCoordinates(c.v2, c.v1).print(buf, sizeof(buf));
		debug_if(MS_DEBUG, "Goto %s \r\n", buf);
